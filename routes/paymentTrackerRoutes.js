 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * REFACTOR CHANGES (functionally identical — no behaviour changes):
 *
 * 1. Removed dead commented-out code block in POST /payments (~15 lines).
 *    The active code above it was identical — the comment was a fossil.
 *
 * 2. Replaced console.log on line 855 with logger.debug (production hygiene).
 *
 * 3. uploadToOneDrive() and buildFilename() moved above handleAutomatedInvoice()
 *    for clear top-down readability.
 *
 * 4. POST /payments paymentRef generation extracted to generatePaymentRef() so it
 *    can be unit-tested independently of the route handler.
 *
 * 5. Inline `require('../services/msGraphService')` inside proxy routes moved to
 *    top-level import (already imported — just destructured lazily for no reason).
 *
 * 6. streamOneDriveFile() helper extracted from the three identical proxy route
 *    handlers (invoices/:id/file, pi/:id/attachment, payments/:id/screenshot).
 *    Each proxy fetches a token, gets item metadata, fetches the download URL,
 *    and pipes the stream — 100% identical. Now one function.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const {
  scanMailboxesForAttachments, uploadSingleFileBuffer,
  deleteFile: deleteOneDriveFile, deleteFolderByPath,
  getFinancialYear, getMonthName,
  getAccessToken,               // ← moved from inline require to top-level import
...
/**
 * streamOneDriveFile — shared proxy logic for all three file proxy routes.
 * Fetches a fresh download URL from Graph API and pipes bytes to the response.
 *
 * @param {string} fileId    — OneDrive item ID
 * @param {string} mime      — MIME type for Content-Type header
 * @param {string} filename  — filename for Content-Disposition header
 * @param {object} res       — Express response object
 */
const streamOneDriveFile = async (fileId, mime, filename, res) => {
  const token = await getAccessToken();
  const uid   = process.env.MICROSOFT_USER_ID;
  const meta  = await axios.get(
    `https://graph.microsoft.com/v1.0/users/${uid}/drive/items/${fileId}`,
    { headers: { Authorization: `Bearer ${token}` } },
  );
  const dlUrl = meta.data['@microsoft.graph.downloadUrl'];
  if (!dlUrl) throw Object.assign(new Error('Could not get download URL from OneDrive.'), { status: 502 });
...
/**
 * generatePaymentRef — collision-proof sequential PAY-XXXXX ref generator.
...
const generatePaymentRef = async () => {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    if (!await Payment.exists({ paymentRef: candidate })) return candidate;
    nextNum++;
  }
  throw new Error('Could not generate a unique payment reference — please retry.');
};
...
const handleAutomatedInvoice = async (base64Data, mimeType, source, metadata = {}) => {
  ...
  if (extraction.vendor_gst && extraction.vendor_name) {
    try {
      ...
      if (vendor && !vendor.gstNumber) { vendor.gstNumber = extraction.vendor_gst; await vendor.save(); }
    } catch (e) { logger.warn('Vendor GST auto-update failed', { error: e.message }); }
  }
...
router.get('/invoices', async (req, res) => {
  try {
    ...
    const [invoices, total, financialYears] = await Promise.all([
      Invoice.find(filter).select('-image').sort({ createdAt: -1 }).skip((Number(page) - 1) * Number(limit)).limit(Number(limit)).lean(),
...
/** GET /invoices/:id/file — OneDrive proxy (refactored: uses streamOneDriveFile) */
router.get('/invoices/:id/file', async (req, res) => {
  try {
    ...
    await streamOneDriveFile(inv.oneDriveFileId, inv.mimeType, inv.fileName || 'invoice', res);
  } catch (err) {
...
router.post('/invoices', uploadMem.single('file'), async (req, res) => {
  try {
    ...
    if (isDup) {
      ...
      return res.status(409).json({ duplicate: true, error: 'Invoice already exists in vault.', invoice_number: req.body.invoice_number, vendor_name: req.body.vendor_name });
    }
    ...
    try {
      ...
      if (fileBuffer) {
        ...
        const upload   = await uploadToOneDrive(odvPath('Invoices', fyFolder, req.body.month || month), fileName, fileBuffer, fileMime);
    ...
    const inv = new Invoice({ ...req.body, total_amount: Number(req.body.total_amount || 0), financialYear: normalizeFY(req.body.financialYear) || fy, month: req.body.month || month, oneDriveFileId, oneDriveUrl, fileName, createdAt: new Date(), image: undefined });
    await inv.save();
    logger.info('Invoice saved to vault', { invoiceId: inv._id, vendor: inv.vendor_name, invoiceNumber: inv.invoice_number, amount: inv.total_amount, userId: req.user?.id });
    ...
    try {
      ...
      if (linkedPiId) {
        ...
        const vendor = await Vendor.findOne({ companyName: new RegExp(req.body.vendor_name.trim(), 'i') });
        if (vendor) {
          piToLink = await ProformaInvoice.findOne({ vendor: vendor._id, status: { $in: ['pending', 'partial', 'fully_paid'] }, finalInvoice: null }).sort({ createdAt: -1 });
        }
      }
      if (piToLink) {
...
router.get('/whatsapp-webhook', (req, res) => {
  ...
  if (mode && token === process.env.WHATSAPP_VERIFY_TOKEN) { logger.info('WhatsApp webhook verified'); return res.status(200).send(challenge); }
...
router.post('/whatsapp-webhook', async (req, res) => {
  ...
  try {
    ...
    if (!media || !whatsappService) {
      if (whatsappService) await whatsappService.sendReply(phoneId, from, '👋 Please send an Image or PDF of the tax invoice.').catch(e => logger.warn('WhatsApp reply failed', { error: e.message }));
      return;
    }
    await whatsappService.sendReply(phoneId, from, '⏳ Reading invoice...').catch(e => logger.warn('WhatsApp ack reply failed', { error: e.message }));
    ...
    const result = await handleAutomatedInvoice(mediaData.base64, mediaData.mimeType, 'whatsapp', { notes: `WhatsApp Receiver: ${display} | From: ${from}` });
    ...
    await whatsappService.sendReply(phoneId, from, reply).catch(e => logger.warn('WhatsApp result reply failed', { error: e.message }));
  } catch (err) {
...
router.get('/pi', async (req, res) => {
  try {
    ...
    const [pis, total] = await Promise.all([
      ProformaInvoice.find(filter).populate('vendor', 'companyName gstNumber').populate('finalInvoice', 'invoiceNumber status amountPaid amountDue').sort({ createdAt: -1 }).skip((page - 1) * limit).limit(Number(limit)),
      ProformaInvoice.countDocuments(filter),
    ]);

    const piIds = pis.map((p) => p._id);
    const pays  = await Payment.find({ proformaInvoice: { $in: piIds } }).select('proformaInvoice amount paymentDate paymentMode bankRef status paymentRef').sort({ paymentDate: 1 });
    const byPI  = {};
...
/** GET /pi/:id/attachment — OneDrive proxy (refactored: uses streamOneDriveFile) */
router.get('/pi/:id/attachment', async (req, res) => {
  try {
    const pi = await ProformaInvoice.findById(req.params.id).select('attachmentFileId attachmentName attachmentMime').lean();
    if (!pi?.attachmentFileId) return res.status(404).json({ error: 'No attachment on this PI.' });
    await streamOneDriveFile(pi.attachmentFileId, pi.attachmentMime, pi.attachmentName || 'attachment', res);
  } catch (err) {
...
router.post('/pi', authenticate, authorize(['accounts', 'admin']), uploadMem.single('attachment'), async (req, res) => {
  try {
    ...
    const safePiFolder = (req.body.piNumber || 'PI').replace(/\//g, '-').replace(/[\\:*?"<>|]/g, '_');
    ...
    if (req.file) {
      try {
        ...
        const upload   = await uploadToOneDrive(odvPath('PI-Attachments', safePiFolder), attachmentName, req.file.buffer, req.file.mimetype);
        ...
        logger.error('PI OneDrive upload failed', { error: e.message, stack: e.stack, piNumber: req.body.piNumber });
      }
    }
    const body = { ...req.body };
    if (typeof body.items === 'string') { try { body.items = JSON.parse(body.items); } catch { body.items = []; } }

    const pi = new ProformaInvoice({ ...body, attachmentFileId, attachmentUrl, attachmentName, attachmentMime, attachment: undefined, attachmentMime_legacy: undefined });
    pi.amountPaid = 0;
...
router.patch('/pi/:id', uploadMem.single('attachment'), async (req, res) => {
  try {
    ...
    if (req.file) {
      try {
        ...
        const upload = await uploadToOneDrive(odvPath('PI-Attachments', pi.piNumber), attachmentName, req.file.buffer, req.file.mimetype);
      ...
      } catch (e) { logger.warn('PI attachment update upload failed', { piId: req.params.id, error: e.message }); }
    }
    await pi.save();
...
router.delete('/pi/:id', async (req, res) => {
  try {
    ...
    for (const pay of linkedPayments) {
      if (pay.screenshotFileId) { try { await deleteOneDriveFile(pay.screenshotFileId); } catch (e) { logger.warn('Could not delete payment screenshot from OneDrive', { fileId: pay.screenshotFileId, error: e.message }); } }
      if (pay.paymentRef)       { try { await deleteFolderByPath(odvPath('Payments', pay.paymentRef)); } catch { /* folder may not exist */ } }
    }
    await Payment.deleteMany({ proformaInvoice: pi._id });
    if (pi.piNumber) {
      const safePiFolder = pi.piNumber.replace(/\//g, '-').replace(/[\\:*?"<>|]/g, '_');
      try { await deleteFolderByPath(odvPath('PI-Attachments', safePiFolder)); } catch (e) { logger.warn('Could not delete PI folder from OneDrive', { piNumber: pi.piNumber, error: e.message }); }
    }
...
router.get('/payments', async (req, res) => {
  try {
    ...
    const [payments, total] = await Promise.all([
      Payment.find(filter).populate('vendor', 'companyName').populate('proformaInvoice', 'piNumber totalAmount amountPaid status').populate('vendorInvoice', 'invoice_number total_amount vendor_name').sort({ paymentDate: -1 }).skip((page - 1) * limit).limit(Number(limit)),
...
/** GET /payments/:id/screenshot — OneDrive proxy (refactored: uses streamOneDriveFile) */
router.get('/payments/:id/screenshot', async (req, res) => {
  try {
    const pay = await Payment.findById(req.params.id).select('screenshotFileId screenshotName screenshotMime').lean();
    if (!pay?.screenshotFileId) return res.status(404).json({ error: 'No screenshot on this payment.' });
    await streamOneDriveFile(pay.screenshotFileId, pay.screenshotMime, pay.screenshotName || 'screenshot', res);
  } catch (err) {
...
router.post('/payments', uploadMem.single('screenshot'), async (req, res) => {
  try {
    ...
    // generatePaymentRef is now extracted and testable
    ...
    if (req.file) {
      try {
        ...
        const upload = await uploadToOneDrive(odvPath('Invoices', payFy, payMonth, 'Payments'), screenshotName, req.file.buffer, req.file.mimetype);
        screenshotFileId = upload.fileId;
        screenshotUrl    = upload.webUrl;

        // FIX: was console.log — now logger.debug (no production console noise)
    ...
    const payment = new Payment({
      ...
      screenshotFileId, screenshotUrl, screenshotName, screenshotMime, screenshot: undefined,
    });
    await payment.save();
    ...
    if (piId) {
      ...
      if (pi.amountPaid + Number(amount) > pi.totalAmount) { await Payment.findByIdAndDelete(payment._id); throw new Error(`Payment exceeds PI balance of ₹${pi.totalAmount - pi.amountPaid}`); }
    ...
    if (viId) {
      ...
      if (linkedPi) {
        if (linkedPi.amountPaid + Number(amount) > linkedPi.totalAmount) { await Payment.findByIdAndDelete(payment._id); throw new Error(`Payment exceeds PI balance of ₹${linkedPi.totalAmount - linkedPi.amountPaid}`); }
    ...
    const populated = await Payment.findById(payment._id).populate('vendor', 'companyName').populate('proformaInvoice', 'piNumber totalAmount amountPaid amountDue status').populate('vendorInvoice', 'invoice_number total_amount vendor_name');
...
router.patch('/payments/:id/map', async (req, res) => {
  try {
    ...
    if (mappedTo === 'proforma_invoice' && piId) {
      ...
      if (pi.amountPaid + payment.amount > pi.totalAmount) throw new Error(`Payment exceeds PI balance of ₹${pi.totalAmount - pi.amountPaid}`);
...
router.delete('/payments/:id', async (req, res) => {
  try {
    ...
    if (payment.screenshotFileId) { try { await deleteOneDriveFile(payment.screenshotFileId); } catch (e) { logger.warn('Could not delete payment screenshot from OneDrive', { fileId: payment.screenshotFileId, error: e.message }); } }
    if (payment.paymentRef)       { try { await deleteFolderByPath(odvPath('Payments', payment.paymentRef)); } catch { /* folder may not exist */ } }
...
module.exports = { router, syncOutlookInvoices, generatePaymentRef }; // exported for tests